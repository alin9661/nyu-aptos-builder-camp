...
export class WalletService {
  ...
  static async generateWallet(): Promise<GeneratedWallet> {
    try {
      ...
      logger.info('Generated new Aptos wallet', {
        address,
  ...
  static async createWalletForUser(
    ...
    try {
      ...
      if (existingUsers.length > 0) {
        ...
        logger.info('User already exists', { ssoId, address: user.address });
        return {
      ...
      logger.info('Created new user with generated wallet', {
        ssoId,
      ...
      logger.error('Failed to create wallet for user', { error, ssoId });
  ...
  static async getAccountForSigning(address: string): Promise<Account | null> {
    try {
      ...
      if (users.length === 0) {
        logger.warn('No auto-generated wallet found for address', { address });
        return null;
      }
      ...
      if (!user.encrypted_private_key) {
        logger.warn('No encrypted private key found', { address });
        return null;
      }
      ...
      logger.info('Retrieved account for signing', { address });
      return account;
    } catch (error) {
      logger.error('Failed to get account for signing', { error, address });
      return null;
    }
  }
  ...
  static async fundWallet(address: string): Promise<string | null> {
    try {
      ...
      if (process.env.APTOS_NETWORK !== 'testnet') {
        logger.warn('Wallet funding only available on testnet', { address });
        return null;
      }
      ...
      logger.info('Funded wallet from faucet', {
        address,
        amount: '1 APT',
        txHash: txn.hash,
      });
      ...
      logger.error('Failed to fund wallet', { error, address });
      return null;
    }
  }
