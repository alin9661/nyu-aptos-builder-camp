...
Unsubscribe: ${unsubscribeUrl}
  ...
  private loadTemplate(filename: string): string {
    try {
      // Use process.cwd() to find templates in Next.js serverless environment
      const templatePath = path.join(process.cwd(), 'lib/templates/emails', filename);

      // Check if template exists, return placeholder if not
      if (!fs.existsSync(templatePath)) {
        logger.warn('Email template not found, using placeholder', { filename, path: templatePath });
